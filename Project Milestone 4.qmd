...
Graph_2 <- ggplot(data_long_pct, aes(x = race_eth, y = value, fill = metric)) +
  ...
  labs(
    ...
    caption = "Interpretation: Data represents percentages calculated based on total population per race/ethnicity.
    \n The graph shows the Race Categories most affected are Alaska Native, White, Black and Hispanic
    \n with Asian and Multiracial least affected.  " # Adding a footnote
  ) +
  scale_fill_manual(
  ...
  theme(
    ...
    plot.caption = element_text(hjust = 0, size = 8)  # Left-align the footnote
  )
...
pop_stats <- pop_aggregate %>%   ungroup() %>%   summarise(Mean_Population = mean(pop_aggregate, na.rm = TRUE),     
Max_Population = max(pop_aggregate, na.rm = TRUE),     Min_Population = min(pop_aggregate, na.rm = TRUE)   )   
...
race_kable <- kable(CA_race_table,
      col.names = c('Race/Ethnicity', 'Percent Infected', 'Percent Severe Infection of the Infected', 'Percent Unrecovered of the Infected'), 
      digits = 1, align = c('l', 'c', 'c', 'c'), 
      caption = "Infection Percentages by Race/Ethnicity") %>% footnote(general_title = "Interpretation:", general = "The group with the highest percent of infection is American Indian or Alaska Native but the group with the highest percent of severe infection and unrecovered infection is white. Multiracial people have the lowest percent of infection, severe infection, and unrecovered infection.")
